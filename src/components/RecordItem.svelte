<script>
    ...
    let {item, checked, deleted} = record;

    const checkedRecord = () => {
        if (checked) {
            $recordCheckedList = $recordCheckedList.filter((r) => r !== record);
            ...
            // remove the record from todolist
            $recordList = $recordList.filter((r) => r !== record);

            // add the record to checked records list
            $recordCheckedList = [
                ...$recordCheckedList,
                {
                    item,
    ...
    const deleteRecord = () => {
        if (deleted) {
            $recordDeletedList = $recordDeletedList.filter((r) => r !== record);
        } else {
            $recordList = $recordList.filter((r) => r !== record);
            $recordCheckedList = $recordCheckedList.filter((r) => r !== record);
...
<div class="form-control">
    <label class="cursor-pointer label">
        <span class="label-text" class:text-success={checked}>{item}</span>
        <div class="flex gap-2">
            {#if deleted}
                <div class="tooltip" data-tip="Ripristina">
                    <button
                        class="checkbox p-1 bg-white"
                        on:click={() => resetRecord()}
                    >
                        <svg
            ...
            {:else}
                <div class="tooltip" data-tip="Completa">
                    <input
                        ...
                        bind:checked
                        class="checkbox"
                        class:checkbox-success={checked}
                        on:click={() => checkedRecord()}
                    />
                </div>
...
<div class="alert" class:visible={openConfirm} class:hidden={!openConfirm}>
    ...
    <div>
        <button class="btn btn-sm" on:click={() => (openConfirm = false)}
            >Annulla</button
        >
        <button class="btn btn-sm btn-error" on:click={() => deleteRecord()}
            >Conferma</button
        >
    </div>
</div>
